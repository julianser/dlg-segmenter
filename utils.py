...
def NormalInit3D(rng, sizeI, sizeX, sizeY, scale=0.01, sparsity=-1):
    """ 
    Normal Initialization for 3D tensor, where sparsity is applied to second and third indices
    """
    sizeI = int(sizeI)
    sizeX = int(sizeX)
    sizeY = int(sizeY)

    values = numpy.zeros((sizeI, sizeX, sizeY), dtype=theano.config.floatX)

    for di in xrange(sizeI):
        values[di, :, :] = NormalInit(rng, sizeX, sizeY, scale, sparsity)
